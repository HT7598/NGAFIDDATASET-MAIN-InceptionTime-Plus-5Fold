...
class NGAFID_Dataset_Downloader:
    ...
    @classmethod
    def _local_dataset_exists(cls, name: str, destination: str = ''):
        base_path = os.path.join(destination, name)

        if name == 'all_flights':
            return (
                os.path.isdir(os.path.join(base_path, 'one_parq'))
                and os.path.exists(os.path.join(base_path, 'flight_header.csv'))
            )
    ...
    def download(cls, name : str, destination:str = '', extract = True):
        ...
        if cls._local_dataset_exists(name, destination):
            logger.info('Using local dataset at {}', os.path.join(destination, name))
